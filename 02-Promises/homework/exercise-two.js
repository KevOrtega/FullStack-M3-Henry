...
function problemC () {
  ...
  filenames.forEach(filename => {
    ...
    promise = promise.then(stanza => {blue(stanza); return promisifiedReadFile(filename)}, err => magenta(new Error(err)))
  })

  promise = promise.then(stanza => {blue(stanza); console.log('done')})
}
...
function problemD () {
  ...
  filenames.forEach(filename => {
    if(filename == filenames[0]) return;
    promise = promise.then(stanza => {blue(stanza); return promisifiedReadFile(filename)})
  })

  promise = promise.then(stanza => blue(stanza)).catch(err => magenta(new Error(err))).finally(() => console.log('done'))
}
