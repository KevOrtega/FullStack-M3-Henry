...
function problemB () {
  ...
  // promise version
  // ???
  ['poem-one/stanza-02.txt', 'poem-one/stanza-03.txt'].forEach(url => {
    return promisifiedReadFile(url)
...
function problemE () {
  ...
  promisifiedReadFile('poem-one/stanza-03.txt')
    .then(stanza => {blue(stanza); return promisifiedReadFile('poem-one/wrong-file-name.txt')})
    .then(stanza => blue(stanza))
    .catch(err => magenta(new Error(err)))
}
